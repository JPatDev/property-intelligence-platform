...
public sealed partial class PostgreSqlWorkflowFixture : IAsyncLifetime
{
    private readonly string _databaseName =
        $"property_intelligence_workflow_tests_{Guid.NewGuid():N}";
    ...
    public async ValueTask InitializeAsync()
    {
        ...
        var testConnectionBuilder = new NpgsqlConnectionStringBuilder(_adminConnectionString)
        {
        ...
        var configuration = new ConfigurationBuilder()
            ...
            {
                ["ConnectionStrings:Workflow"] = testConnectionBuilder.ConnectionString,
                ["Workflow:Operations:ScanInterval"] = "01:00:00",
            })
            .Build();
        ...
        var claimsDbContext = Services.GetRequiredService<ClaimsDbContext>();
        await claimsDbContext.Database.MigrateAsync();
        var documentsDbContext = Services.GetRequiredService<DocumentsDbContext>();
    ...
    private static string ReadPasswordFile(NpgsqlConnectionStringBuilder connection)
    {
        ...
        if (!File.Exists(passfilePath))
        {
            throw new InvalidOperationException(
                ...
                "ConnectionStrings__WorkflowTestAdmin or pgpass.conf.");
        }
        ...
        throw new InvalidOperationException(
            "No pgpass.conf entry matches the PostgreSQL workflow test connection.");
    }
    ...
    [GeneratedRegex("^property_intelligence_workflow_tests_[a-f0-9]{32}$")]
